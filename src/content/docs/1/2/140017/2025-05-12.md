---
description:
  Definizioni di curve chiuse e semplici, vettore velocità, retta tangente,
  curve regolari e cambiamento di parametro.
lang: it
title: Curve in R^n, definizione, vettore velocità e retta tangente
---

## Curve in $\R^n$

:::tip

- integrale doppio: formalizza il concetto di area;
- integrale triplo: formalizza il concetto di volume;
- integrale curvilineo: formalizza il concetto di lunghezza di una curva;

:::

Una **curva** (o traiettoria) è una mappa $\gamma: I \subseteq \R \to \R^n$,
continua su $I$, $\gamma(t) = (\gamma_1(t), \ldots, \gamma_n(t))$, con
$\gamma_i: I \to \R$.

- Se $I = [a, b]$, allora i punti $\gamma(a)$ e $\gamma(b)$ si chiamano estremi
  della curva.
- L'insieme $\gamma(I) \subseteq \R^n$ si chiama sostegno della curva.
- L'equazione $\mathbf{x} = (x_1, \ldots, x_n) = \gamma(t)$ è detta equazione
  parametrica di $\gamma$.
- La curva si dice **chiusa** se $I = [a, b]$ e $\gamma(a) = \gamma(b)$.
- Una curva si dice **semplice** se $\gamma$ è iniettiva, oppure se $\gamma$ è
  chiusa e $\gamma: [a,b) \to \R^n$ è iniettiva.

:::note

Due curve possono avere lo stesso sostegno ma essere differenti.

**Esempio**: Due curve che hanno come sostegno una circonferenza, ma i cui
domini sono $[0, 2 \pi]$ e $[0, 4 \pi]$. In questo caso una curva è semplice e
l'altra no.

:::

### Orientazione (o verso) di una curva semplice

Data $\gamma: I \to \R^n$ curva semplice, si dice che il punto
$\mathbf{x}_1 = \gamma(t_1)$ precede il punto $\mathbf{x}_2 = \gamma(t_2)$ se
$t_1 < t_2$.

### Vettore velocità di una curva

Sia $\gamma: T \to \R^n$ una curva,
$\gamma(t) = (\gamma_1(t), \ldots, \gamma_n(t))$. Se le componenti $\gamma_i$
sono derivabili in un punto $t_0 \in I$, il vettore velocità di $\gamma$ in
$t_0$ è definito:

$$
\gamma'(t_0) = (\gamma'_1(t_0), \ldots, \gamma'_n(t_0))
$$

Possiamo scrivere:

$$
\gamma(t) = \gamma(t_0) + \gamma'(t_0)(t - t_0) + o(t - t_0) \text{ per } t \to t_0
$$

Se $\gamma'(t_0) \neq \mathbf{0}$, allora si chiama **retta tangente** alla
curva $\gamma$ nel punto $x_0 = \gamma(t_0)$ l'equazione parametrica:

$$
\mathbf{x} = \gamma(t_0) + \gamma'(t_0)(t - t_0)
$$

Se $\gamma'(t_0) = \mathbf{0}$, la retta tangente **può** non esistere.

### Curva regolare

Una curva si dice di classe $C^m$ se tutte le $\gamma_i$ sono di classe $C^m$.

$\gamma: I \to \R^n$ si dice **regolare** se è di classe $C^1$ e
$\forall\ t \in I,\ \gamma'(t) \neq \mathbf{0}$.

Data $\gamma$ curva regolare, si chiama **versore tangente** a $\gamma$ il campo
vettoriale:

$$
T_\gamma(t) = \frac{\gamma'(t)}{||\gamma'(t)||},\ t \in I
$$

#### Curva regolare a tratti

Una curva $\gamma$ si dice $C^1$ a tratti (o regolare a tratti) se esiste una
suddivisione tale che in ogni suo intervallo $\gamma$ sia di classe $C^1$.

In tal caso $\gamma$ è considerata unione delle $N$ curve regolari $\gamma_i$ e
si scrive:

$$
\gamma = \cup_{i = 1}^N \gamma_i
$$

### Cambiamento di parametro di una curva

Due curve di classe $C^1$, $\gamma: I \to \R^n$ e
$\tilde{\gamma}: \tilde{I} \to \R^n$, sono dette **equivalenti** se esiste una
funzione biettiva $\phi: \tilde{I} \to I$ tale che:

- $\phi \in C^1(\tilde{I})$
- $\forall\ \tau \in \tilde{I},\ \phi'(\tau) \neq \mathbf{0}$
- $\forall\ \tau \in \tilde{I},\ \tilde{\gamma}(\tau) = \gamma(\phi(\tau))$.

In tal caso, la funzione $\phi$ è chiamata cambiamento di parametrizzazione.

Se $\forall\ \tau \in \tilde{I},\ \gamma'(\tau) > 0$, $\gamma$ e
$\tilde{\gamma}$ hanno lo stesso verso. Invece se
$\forall\ \tau \in \tilde{I},\ \gamma'(\tau) < 0$, $\gamma$ e $\tilde{\gamma}$
hanno verso opposto.

Due curve equivalenti hanno lo stesso sostegno; se una delle due è semplice,
anche l'altra lo è.
