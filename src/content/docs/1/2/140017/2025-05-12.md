---
description:
  Curve in R^n,  concetti chiave come curve chiuse, semplici, il vettore
  velocità e la retta tangente. Curve regolari e cambiamento di parametro tra
  curve equivalenti.
lang: it
...
- Se $I = [a, b]$, allora i punti $\gamma(a)$ e $\gamma(b)$ si chiamano estremi
  della curva.
- L'insieme $\gamma(I) \subseteq \R^n$ si chiama sostegno della curva.
...
**Esempio**: Due curve che hanno come sostegno una circonferenza ma i loro
...
$\mathbf{x}_1 = \gamma(t_1)$ precede il punto $\mathbf{x}_2 = \gamma(t_2)$ se
$t_1 < t_2$.
...
sono derivabili in un punto $t_0 \in I$, il vettore velocità di $\gamma$ in
$t_0$ è definito:
...
In tal caso $\gamma$ è considerata unione delle $N$ curve regolari $\gamma_i$ e
si scrive:
...
Due curve di classe $C^1$, $\gamma: I \to \R^n$ e
$\tilde{\gamma}: \tilde{I} \to \R^n$, sono dette **equivalenti** se esiste una
...
In tal caso, $\phi(\tau) = \tau \to t$ è chiamato cambiamento di
parametrizzazione.

Se $\forall\ \tau \in \tilde{I},\ \gamma'(\tau) > 0$, allora $\gamma$ e
$\tilde{\gamma}$ hanno lo stesso verso. Se
$\forall\ \tau \in \tilde{I},\ \gamma'(\tau) < 0$, allora $\gamma$ e
