...
$$
\begin{dcases}
C \dv{V_C}{t}(t) = \frac{V_s - V_C(t)}{R_1} \\
V_C(t_0) = 0
\end{dcases}
$$
...
$$
\begin{dcases}
\dv{V_C}{t}(t) = \frac{V_s - V_C(t)}{\tau} \iff \int_{V_C(t_0)}^{V_C(t)} \frac{dV_C(t)}{V_s - V_C(t)} = \frac{1}{\tau} \int_{t_0}^t dt \\
V_C(t_0) = 0
\end{dcases}
$$
...
$$
\begin{dcases}
\left[- \ln(V_s - V_C(t))\right]_{V_C(t_0)}^{V_C(t)} = \frac{t}{\tau} \\
V_C(t_0) = 0
\end{dcases}
$$
...
- $V_C(\tau) = V_s (1 - \frac{1}{e})$
- $V_C(4 \tau) = V_s (1 - \frac{4}{e})$: vicino al 98% della tensione massima.
