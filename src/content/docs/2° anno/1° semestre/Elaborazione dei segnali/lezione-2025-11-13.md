...
$$
x(n T_c) = x_q(n T_c) + \epsilon_q(n T_c))
$$

:::caution

L'errore (o rumore di quantizzazione) $\epsilon$ è inevitabile: il campionamento
è sempre un'operazione lossy.

:::

Il numero di livelli viene solitamente scelto come una potenza di $2$, in modo
che $2^M$ livelli siano rappresentabili con $M$ bit. Il livello che viene
...
errore che posso commettere su un campione è $|\frac{\Delta}{2}|$ (se il segnale
in ingresso varia tra $\pm 1$, allora l'errore massimo è $\frac{1}{M}$.).
...
variabile aleatoria distribuita uniformemente in
$[- \frac{\Delta}{2}, \frac{\Delta}{2}]$.

Si può quindi stimare la potenza media del rumore che coincide con la varianza:

$$
\sigma_q^2 = \int_{-\frac{\Delta}{2}}^{\frac{\Delta}{2}} \epsilon^2\ f_{\epsilon_q}(\epsilon)\ d\epsilon
...
$$
SNR = 10 \log_{10}\left(\frac{P_x}{P_\epsilon}\right)
$$
...
Per realizzarlo bisogna ottimizzare congiuntamente la posizione dei punti di
transizione e quella dei livelli rappresentativi in modo da minimizzare l'errore
quadratico medio del quantizzatore.
...
1. Si impostano i punti di soglia $b_j, j=1,\ldots,n-1$ ai valori centrali tra
   livelli successivi: $b_j = \frac{1}{2} (a_{j + 1} + a_j)$.
2. Si ricalcolano i livelli di quantizzazione $a_i, i=1,\ldots,n$ imponendoli
   pari alla media della differenza di potenziale in $[b_{i - 1}, b]$, con
   $b_0 = -\infty$ e $b_n = +\infty$.
3. Si calcola l'errore quadratico medio confrontandolo con il ciclo precedente.
