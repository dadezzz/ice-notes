...
Alla base dei circuiti MOSFET si trova una struttura a strati:
...
La superficie del semiconduttore è in accumulo.
...
La carica positiva sul gate è bilanciata da quella negativa degli ioni accettori
...
La carica positiva sul gate è bilanciata sia dagli ioni accettori, sia dagli
elettroni.
...
- $V_{GS} \ll V_T$: diodi polarizzati a zero o inversamente, quindi al massimo
  scorre una corrente molto debole tra di essi;
- $V_{GS} < V_T$: si forma la regione di svuotamento sotto il gate, che si
  congiunge con quelle dovute alle giunzioni $pn$, ma non scorre corrente perché
  essa è costituita da ioni fissi;
...
Quindi se $V_{GS}$ è sotto la tensione di soglia l'interruttore è aperto,
altrimenti è chiuso.

### Regione lineare (triodo)

Se la tensione sull'ossido è superiore a quella di soglia in ogni suo punto
($V_{GS}  \geq V_{DS} + V_T$), con $V_DS$ molto piccola, all'interno del canale
scorre una corrente di drift che si calcola con:
