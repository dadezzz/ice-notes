...
also has to **control all other programs** and to block malicious apps from
...
to give it back. CPUs support setting a timer that generates an interrupt. The
kernel executes the interrupt and can thus decide to let the current process run
or to give space to another one.
