...
- **Array**: it's the simplest structure, the main memory is physically an array
  of bytes. It's disadvantages are the fixed size and the difficulty in removing
  and adding items in the middle.
- **List**: it's most commonly implemented as a linked list and it allows to
  change its size and to insert and remove items in the middle.
