...
- **Array**: it's the simplest structure; the main memory is physically an array
  of bytes. Its disadvantages are the fixed size and the difficulty in removing
  and adding items in the middle.
- **List**: it's most commonly implemented as a linked list and it allows
  changing its size and inserting and removing items in the middle.
- **Stack**: it's a sequentially ordered data structure using LIFO for adding
  and removing items. It's used in the memory to store local variables when a
  function is called.
- **Queue**: a sequentially ordered data structure using FIFO. It's often used
  for buffers.
...
- ABI: definitions of syscall parameters and their number, they must correspond
...
2. it determines if it has to load other dynamic libraries (.dll on Windows, .so
   on UNIX);
...
Registers are small and in a limited number but extremely fast memory locations.
...
- when the interrupt fires, the kernel regains control and can decide to let the
  process continue executing or to do a context switch to another process;
