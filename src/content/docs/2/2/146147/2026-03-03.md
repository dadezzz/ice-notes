---
description:
  Process states, the structure of the Process Control Block (PCB), context
  switching, threading, scheduling algorithms, and the lifecycle of processes
  like zombie and orphan processes.
lang: en
...
Context switching is time that is 'wasted' because everything is blocked from
executing except for the kernel. The more complicated the kernel and the PCB
are, the more time it takes to switch context.

## Threads

So far we have considered processes with a single execution flow.

To execute more tasks in parallel in a single program, we need to create
multiple **threads**.

Each thread has an independent PC, stack, and set of registers assigned. A
process must support storing all this data in the PCB. The extension of the PCB
used to store thread-specific information is called Thread Control Block (TCB).
...
the `next` pointer to the process scheduled to run immediately after the current
one.
...
another program, the child process must call `exec()` with the executable name
...
call `wait()`, the OS retains the PCB in memory, and the child process becomes a
**zombie process**.
